...
def build_usdjpy_features(df_m15, df_m30, df_h1, df_h4):
...
def create_target(df, forward_bars=5, threshold_pips=10):
    ...
    # USDJPY: price around 150, so 10 pips = 0.10
    # But in percentage terms: 0.10 / 150 = 0.00067 (0.067%)
    # Let's use percentage threshold instead
...
def plot_monte_carlo_results(results):
    ...
    fig.suptitle('USDJPY Ensemble Model - Monte Carlo Simulation Results', fontsize=16)
...
def print_monte_carlo_summary(results):
    ...
    print("\n" + "="*70)
    print("MONTE CARLO SIMULATION RESULTS - USDJPY ENSEMBLE")
    print("="*70)
